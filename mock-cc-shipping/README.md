...
A standalone, zero-dependency mock of the real **Shipping API | Collector Crypt** so the Hoshi
redeem flow (Track A/B) can be dry-run locally with **no CC credentials**.
...
* `server.mjs` — the whole server. Node built-ins only (`node:http`, `node:crypto`). No npm install.
* `smoke.mjs` — a 56-assertion walk of the happy path and every documented error case. It calls
  `POST /__mock/reset` first, so do not run it against a server holding a dry-run you still need.

Nothing in here is imported by the backend; it is a separate process the backend talks to over HTTP.
...
```bash
# from hoshi-backend/
node mock-cc-shipping/server.mjs
# -> listening http://localhost:4010

# in another terminal, prove it works:
node mock-cc-shipping/smoke.mjs
# -> 56 passed, 0 failed
```

Or via the package scripts: `npm run mock:cc-shipping` / `npm run mock:cc-shipping:smoke`.
...
| `MOCK_LOG` | `true` | per-request log line |
...
The "unsigned transaction" blobs are deterministic base64 markers, not real Solana transactions, so a
wallet cannot actually sign them. The mock matches a submitted leg on the marker
(`shipment / batch / leg kind / leg index`), so a harness may post the blob back **unchanged**, or
re-encode the decoded JSON with extra fields (e.g. `"sig"`) — both are accepted. Everything else about
the leg accounting is real: completeness, batch identity, the 15-minute hold, duplicates.
...
* **Real Solana** — no blockchain, no co-sign, no broadcast. The blobs are markers, and burn results
  are fabricated signatures. The doc's on-chain consequences ("omit the de-list legs and the burn legs
  fail on chain") cannot happen here; the mock refuses an incomplete set up front instead.
...
* **Undocumented shapes** are marked `DOC-SILENT:` in `server.mjs`. The main ones: the list envelopes
